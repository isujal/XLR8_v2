...
//@TeleOp
@Config
public class tag_final extends LinearOpMode {
...
//    private double fieldTargetHeading = 0;
    ...
    private  double enc_angle = 0;
    private double enc_angle_counts = 0;
...
//        enc = hardwareMap.get(Encoder.class, "enc");
        ...
        initAprilTag();
//        robot.turret1.setDirection(DcMotorSimple.Direction.FORWARD);
//        robot.turret2.setDirection(DcMotorSimple.Direction.FORWARD);
//        robot.resetEncoder();
...
//            robot.resetEncoder();
        ...
        while (opModeIsActive()){
            double x_robot = drive.localizer.getPose().position.x;
            double y_robot = drive.localizer.getPose().position.y;
            double deltaX = targetX - x_robot;
            double deltaY = targetY - y_robot;
            DESIRED_ANGLE = Math.atan2(deltaY, deltaX);
            DESIRED_ANGLE = DESIRED_ANGLE - drive.localizer.getPose().heading.toDouble();
            DESIRED_ANGLE = Math.toDegrees(Math.atan2(Math.sin(DESIRED_ANGLE), Math.cos(DESIRED_ANGLE)));
...
//            Pose2d pose = drive.localizer.getPose();
            ...
            telemetry.addData("Pitch,", "%.2f", detections.get(0) );
            telemetry.addData("Roll", "%.2f",detections.get(0).ftcPose.y );
            ...
            if (tagVisible) {
                ...
                telemetry.addData("robot yaw", "%.2f", detections.get(0).robotPose.getOrientation().getYaw(AngleUnit.DEGREES));
                telemetry.addData("robot pitch", "%.2f", detections.get(0).robotPose.getOrientation().getPitch(AngleUnit.DEGREES));
                telemetry.addData("robot roll", "%.2f", detections.get(0).robotPose.getOrientation().getRoll(AngleUnit.DEGREES));
...
//                tagYawDeg = detections.get(0).ftcPose.yaw;
                ...
                telemetry.addData("Pitch," ,"%.2f", detections.get(0) );
                telemetry.addData("DESIRED ANGLE y", "%.2f", DESIRED_ANGLE);                                                                                        
                telemetry.addData("Roll", "%.2f",detections.get(0).ftcPose.y );
                pos = map (detections.get(0).ftcPose.range, hood_min,hood_max,0,1);

//                run_turret(angle + offset + DESIRED_ANGLE , 0, 27845, actualPos, telemetry);
...
//            run_turret(angle + DESIRED_ANGLE , 0, 27845, actualPos, telemetry);
