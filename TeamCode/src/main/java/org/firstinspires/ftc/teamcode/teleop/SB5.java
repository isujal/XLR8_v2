...
import org.firstinspires.ftc.teamcode.prototyping.WaitForSensorAction;
...
import org.firstinspires.ftc.teamcode.subsystem.Endgame;
...
@Config
@TeleOp(name = "TELEOP SB5", group = "ATELEOP")
public class SB5 extends LinearOpMode {
    private static RobotHardware robot=RobotHardware.getInstance();

    public static double pose_x;
    public static int timeoutMs = 3;
...
//    public static boolean nearFlag = false;
    ...
    private AbstractCollection<SequentialAction> scheduler;
...
//                .build();
        ...
        while (opModeIsActive()){
            ...
            drive.setDrivePowers(new PoseVelocity2d(
                    new Vector2d(
                            mapCubic(-gamepad1.left_stick_y*forward),
                            mapCubic(-gamepad1.left_stick_x*strafe)
                    ),
                    mapCubic
...
//            }
            ...
            Action waitForBeam = new WaitForSensorAction(
                    () -> robot.intakeBeam.getState(),
                    3,   // timeout 3s
                    100,    // debounce 100ms
                    false,  // wait for TRUE
                    false   // no edge detection
            );
...
//            telemetry.addData("con : ", feedButtonFlag && zeroFlag);
    ...
    public static double mapCubic(double input) {
        double deadband = 0.05;

        if (Math.abs(input) < deadband) return 0.0;

        // remove deadband
        input = (Math.abs(input) - deadband) / (1.0 - deadband) * Math.signum(input);

        double cubicWeight = 0.7; // ↑ smoother pickup (0.6–0.8 recommended)

        return cubicWeight * Math.pow(input, 3)
                + (1.0 - cubicWeight) * input;
    }


    public static double expo(double x) {
        double expo = 0.85; // 0 = linear, 1 = very soft
        return Math.signum(x) * (expo * Math.pow(Math.abs(x), 3) + (1 - expo) * Math.abs(x));
    }
