...
@Config
@TeleOp(name = "new teleop", group = "ATELEOP")
...
//    public static boolean intakeToggle = false;
    ...
    public static boolean twoStoreIntake = false;
    public static boolean fullIntake = false;
...
//                    robot.turret1.setPower(-val_tag);
            ...
            boolean seqEdge = currentGamepad1.dpad_up && !previousGamepad1.dpad_up;

            feedSequenceFSM.update(seqEdge, feeder, intake);
...
//            lastOBeamstate = currentOBeamstate;
            ...
            telemetry.addData("IntakeState", intakeFSM.getState());
            telemetry.addData("StoredCount", intakeFSM.getStoredCount());

            telemetry.addData("ball", ball);
            telemetry.addData("twostore", twoStoreIntake);
            telemetry.addData("fullintake", fullIntake);
//            telemetry.addData("storedCount", IntakeFSM.storedCount);
...
//    public void scoreArtifact2(double thresh){
//        if( i < ball + 1) {
//            if ((Math.abs(robot.shooter.getVelocity() - thresh) > 25)) {
//                runningActions.add(
//                        new SequentialAction(
////                                Outtake.HoodCommand(Outtake.HoodState.FAR),
//                                Outtake.ShooterCommand(Outtake.ShooterState.TELE_FAR)
...
//                insideDiff = true;
//            } else {
//                if(shooterTimer.getElapsedTimeSeconds() > 0.5 && !insideDiff){
//                    countDone = false;
//                }
//                scheduler.add(new ParallelAction(
//                                new SequentialAction(
//                                        intake.FeederAction(Intake.FeederState.FEED_ON)
//
//                                ),
//                                intake.IntakeAction(Intake.IntakeState.INTAKE_SHOOT),
//                                intake.ClutchAction(Intake.ClutchState.INTAKE)
//                        )
//                );
//                updateCount();
//
...
//        }
        ...
        public static void collectArtifacts2(){
        if(!robot.outtakeBeam.getState() && robot.feederBeam.getState() && robot.intakeBeam.getState()){
            ball = 1;
        }
        else if(ball == 1 && !robot.feederBeam.getState() && robot.intakeBeam.getState()){
            ball = 2;
            twoStoreIntake = true;
        }
        else if(ball == 2 && !robot.intakeBeam.getState()){
            ball = 3;
            fullIntake = true;
        }
        else if(robot.outtakeBeam.getState() && robot.feederBeam.getState() && robot.intakeBeam.getState()){
            ball = 0;
        }
